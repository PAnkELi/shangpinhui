                        ...
                        <div class="priceArea">
                            ...
                            <div class="priceArea2">
                                ...
                                <div class="fixWidth">
                                    ...
                                    <em class="t-gray"> 活动截止至 {{skuInfo.createTime}}</em>
                                </div>
                            </div>
                        </div>
                        <div class="support">
                            ...
                            <div class="supportArea">
                                ...
                                <div class="fixWidth">山西省 晋中市 榆次区</div>
                            </div>
                        </div>
                    </div>
                    <!-- 商品属性选择区域 -->
                    <div class="choose">
                        ...
                        <div class="choosed"></div>
                            <dl v-for="attr in spuSaleAttrList" :key="attr.id">
                                <dt class="title">选择{{attr.saleAttrName}}</dt>
                                <dd 
                                    changepirce="0" 
                        ...
                        <div class="cartWrap">
                            <div class="controls">
                                <!-- Vue2中v-model:xxx 写法不行 -->
                                <input autocomplete="off" class="itxt" v-model="skuNum" @change="handlerSkuNum">
                                <a href="javascript:" class="plus" @click="skuNum++"> + </a>
                                <a href="javascript:" class="mins" @click="skuNum > 1 ? skuNum-- : 1"> - </a>
                            </div>
                            <div class="add" @click="addShoppingCart">
...
<script>
    ...
    import GoodsZoom from './GoodsZoom'
    import {mapGetters} from 'vuex'
    export default {
        name: 'GoodsDetail',
        components: {ImageList, GoodsZoom},
        data() {
            return {
                skuNum: 1,
            }
        ...
        mounted() {
            ...
            this.$store.dispatch('detailAbout/getGoodsDetail', this.$route.params.skuid)
        },
        methods: {
            // 动态给当前选择的属性名添加active类名
            // 这里要注意是地址传递 因此store仓库中的数据也变了
            changeActive(selectedAttr, attrList) {
                attrList.forEach(attr => {
                    attr.isChecked = '0'
                })
                selectedAttr.isChecked = '1'
            },
            // 处理用户输入的商品数量
            // 要考虑两种特殊情况 用户输入的是非数字 用户输入的小数、负数
            handlerSkuNum() {
            ...
            async addShoppingCart() {
                // 发请求通知服务器进行存储
                try {
                    await this.$store.dispatch('detailAbout/getAddOrUpdateCart', {
